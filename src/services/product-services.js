...
class ProductService {
    ...
    getHighlights = async () => {
        ...
        try {
            let pool   = await sql.connect(config);
            let result = await pool.request().query(`
            SELECT TOP 8 P.idProduct, C.Category, S.SubCategory, I.Image, P.Title, P.Price, P.Description, P.Enabled, P.Material, SUM(PO.Quantity) AS TotalQuantitySold
            FROM Product P
            INNER JOIN Category AS C
                ON C.IdCategory = P.IdCategory
            INNER JOIN SubCategory AS S
            ...
            INNER JOIN ImageProduct AS I 
                ON I.idProduct = P.idProduct
            JOIN ProductxOrder PO
                ON P.IdProduct = PO.IdProduct
            GROUP BY P.idProduct, C.Category, S.SubCategory, I.Image, P.Title, P.Price, P.Description, P.Enabled, P.Material
